<template>
    <div class="edit-module">
        ...
        <b-modal centered  v-model="signModalShowModal" title="提示" @ok="signHandle" @cancel="signModalShow = -1" ok-title="确定" cancel-title="取消">
            <div class="modal-msg">
        ...
        <!-- 新建文件 填写名字 -->
        <b-modal centered  v-model="newTempOkShow" title="提示" @ok="newTempOk" @cancel="newTempOkShow = false" ok-title="确定" cancel-title="取消">
            <div class="modal-msg">
        ...
        <!-- 选择模版弹出层 -->
        <b-modal centered  v-model="templateModalShow" title="应用模板" :ok-disabled="templates.length === 0" @ok="selectTemplate" @cancel="templateModalShow = false" ok-title="确定" cancel-title="取消">
            <div class="template-modal-content" v-loading="tempsLoading">
                ...
                <div v-else class="max-hight-box" >
                    <b-form-radio-group v-model="selectedDoc" :options="templates" stacked>
                    </b-form-radio-group>
                </div>
            </div>
        </b-modal>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        watch: {
            ...
            'signModalShow': {
                handler() {
                    this.signModalShowModal = this.signModalShow > -1;
                },
                deep: true
            },
            $route() {
        ...
        methods: {
            ...
            handleShowTemp() {
                ...
                experimentService
                    ...
                    .then(data => {
                        this.templates = data.map(item => { return {'text': item.name, 'value': item}});
                        if (data.length === 0) {
                            this.template = ''
                        }
