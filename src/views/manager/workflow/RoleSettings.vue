                ...
                <div class style="height: 460px; border: 1px solid #eaeaea; overflow: auto">
                    ...
                    >
                        ...
                        <template slot="job_type" slot-scope="row">
                            <span
                                v-if="!row.item.edit"
                            >{{row.item.job_type && row.item.job_type.name || '无职务'}}</span>
                            <b-form-select
                                v-else
                                size="sm"
                    ...
                    >
                        ...
                        <tbody role="rowgroup">
                            <template v-for="(roleGroupAllocation, key) in roleGroupAllocations">
                                ...
                                <template
                                    v-for="(roleAllocation, index) in roleGroupAllocation.role_allocations"
                                >
                                    <tr
                                        class="tr-allocation"
        ...
        savePage() {
            ...
            for (let nodeIndex in this.nodeRoleGroupAllocations) {
                for (let typeIndex in this.nodeRoleGroupAllocations[
                    ...
                    for (let roleIndex in this.nodeRoleGroupAllocations[
                        nodeIndex
                    ][typeIndex].role_allocations) {
                        if (
                            !this.nodeRoleGroupAllocations[nodeIndex][typeIndex]
                                .role_allocations[roleIndex].allocations
                        )
                            continue;
                        for (let index in this.nodeRoleGroupAllocations[
                            nodeIndex
                        ][typeIndex].role_allocations[roleIndex].allocations) {
                            let allocation = _.pick(
                                this.nodeRoleGroupAllocations[nodeIndex][
                                    typeIndex
                                ].role_allocations[roleIndex].allocations[
                                    index
                                ],
