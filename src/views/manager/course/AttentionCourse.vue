    ...
    <div class="attention-course">
        ...
        <b-row class="cardDiv">
            ...
            <b-col md="7" class="align-self-center">
                <b-button-group class="float-right" v-if="isActionAllowed('code_user_management', 'code_user_review_user')">
                    <b-button :size="template_size" class="styledBtn fontedBtn" variant="outline-primary" @click="requestAttentionOpen">签订关注许可</b-button>
                    <b-button class="styledBtn fontedBtn" :size="template_size" variant="outline-primary" @click="cancelRequest">取消关注协议</b-button>
                </b-button-group>
            </b-col>
        </b-row>
        <div class="cardDiv">
            <b-table :items="allData.list" small hover :fields="columns" head-variant style="fontSize: 18px">
                ...
                <template slot="action" slot-scope="row">
                    <b-button class="styledBtn" :size="template_size" variant="outline-primary" @click="viewDetail(row)" v-if="isActionAllowed('code_user_management', 'code_user_review_user')">
        ...
        <b-modal hide-footer centered id="requestModal" ref="requestModal" title="关联指导者">
            <div>
                ...
                ></b-form-textarea>
                <b-button variant="success" @click="sendRequestData" class="mt-5" :disabled="requestData.targetCompany === null">确&emsp;定</b-button>
            </div>
        </b-modal>
        <b-modal size="lg" hide-footer centered id="detailView" ref="detailView" title="查看详情">
            <b-container class="modalContainer" v-if="selectedData">
                <b-row>
                    <b-col lg="6" md="4" sm="4" class="text-left">
                        序号：{{selectedData.id}}
                    </b-col>
                    <b-col lg="6" md="4" sm="4" class="text-left">
                        状态：{{selectedData.status}}
                    </b-col>
                </b-row>
                <b-row>
                    <b-col lg="12" md="4" sm="12" class="text-left">
                        单位名称：{{selectedData.linked_company}}
                    </b-col>
                </b-row>
                <b-row>
                    <b-col md="6" sm="12" class="text-left">
                        创建人：{{selectedData.created_by}}
                    </b-col>
                    <b-col md="6" sm="12" class="text-left">
                        创建时间：{{selectedData.create_time}}
                    </b-col>
                </b-row>
                <b-row>
                    <b-col lg="12" md="12" sm="12" class="text-left">
                        关注协议：{{selectedData.message}}
                    </b-col>
                </b-row>
                <b-row>
                    <b-col md="6" sm="12" class="text-left">
                        签订人：{{selectedData.setter}}
                    </b-col>
                    <b-col md="6" sm="12" class="text-left">
                        签订时间：{{selectedData.seted_time}}
                    </b-col>
                </b-row>
            </b-container>
        </b-modal>
    ...
    export default {
            ...
            return {
                ...
                columns: {
                    ...
                    linked_company: {
                        label: "单位名称",
                        sortable: false,
                        class: "text-center field-linked_company"
                    },
                    status: {
                        label: "状态",
                        sortable: false,
        ...
        methods: {
            queryDataList() {
                this.run();
                CourseService
                    .getInitAttentionData({...this.queryParam, ...this.queryDebounceParam})
                    .then(data => {
            ...
            cancelRequest() {
                this.selected = this.selected.filter(item=>this.allData.list.filter(x=>x.id===item)[0].status === '已关注');
