    ...
    <div class="cardDiv">
      <b-table :items="projects.list" responsive small hover :fields="columns" head-variant>
        ...
        <template slot="edit_control" slot-scope="row">
          ...
          <a class="btn-link mx-1" href="javascript:" @click="viewXmlHandler(row.item.flow)">
            <icon name="eye"></icon>
          </a>
          <a
            class="btn-link mx-1"
  ...
  methods: {
    ...
    checkedItems() {
      return this.projects.list.filter(
        item => item.checked === true && item.edit_able === 1
      );
    },
    // 查看渲染动画的大图
    ...
    handleDownload() {
      ...
      import("@/components/UploadExcel/Export2Excel").then(excel => {
        ...
        if (list.length === 0) list = this.projects.list;
