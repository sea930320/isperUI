  ...
  <div>
    ...
    <b-form @submit="onSubmit">
      <b-container fluid>
          ...
          <b-col sm="5">
            <div class="cardDiv">
              ...
              <template slot="appMode" slot-scope="row">
                ...
                <a href="javascript:;" class="case-text" @click="showBigProcessImg(row.item.process.image)">{{ row.item.process.name}}</a>
              </template>
              <template slot="agreeBit" slot-scope="row">
                <b-form-checkbox v-model="row.item.checked" @change="changeCheckBox($event, row.item,1)">
              ...
              <table class="table table-hover">
                ...
                <template v-for="(type, index) in projectRolesAssign.project_role_type" v-key="index">
                  ...
                  <tr v-for="(role, indexRole) in assignDataForType(type)" :key="indexRole">
                    ...
                    <td width="15%">
                      <b-form-checkbox v-model="role.can_terminate" @change="checkRoleAssign(role)" @input="checkRoleAssign(role)">
                      </b-form-checkbox>
                    </td>
                    <td width="20%">
                      <b-form-checkbox>
        ...
        <b-modal id="roleModal" :visible="roleImageModalShow" title="角色形象" class="my-modal" @ok="changeRoleImage" ok-title="确定" cancel-title="取消" @cancel="resetModal" >
          <b-container>
...
//    import roleImageModal from 'pages/common/roleImageModal';
//    import RoleImageModal from "@/components/roleModal/roleImageModal";
...
//                        project_id: this.$route.params.project_id,
            ...
            onSubmit(e){
                ...
                if (this.nextBtnClicked == 1){
                    this.$router.push({name: 'create-project-wizard3', params: { project_id: this.project_data.id }});
//                    this.$router.push({name: 'create-project-wizard3', params: { project_id: 2035 }});
                }
                ...
                if (this.previousBtnClicked == 1){
                    this.$router.push({name: 'create-project-wizard1', params:{currentProject:this.project_data,is_edit:this.is_edit}});
                }
            },
            nextPage(){
...
<style type="text/css" lang="scss" rel="stylesheet/scss" scoped>
  ...
  .fixed-table thead, tbody { display: block; }
  .fixed-table tbody {
