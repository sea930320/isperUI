        ...
        <b-row class="cardDiv">
            ...
            <b-col md="7" class="align-self-center">
                <b-button-group class="float-right">
                    <b-button class="styledBtn fontedBtn" :size="template_size" variant="outline-primary" @click="()=>{this.$refs['uploadExcel'].show(); this.tableData=[]; this.tableHeader=[]}">导入人员</b-button>
                    <b-button class="styledBtn fontedBtn" :size="template_size" variant="outline-primary" @click="handleDownload">导出人员</b-button>
                    <b-button class="styledBtn fontedBtn" :size="template_size" variant="outline-primary" @click="()=>{
                    ...
                    <b-button class="styledBtn fontedBtn" :size="template_size" variant="outline-primary" @click="this.deleteUsers">删除人员</b-button>
                </b-button-group>
            </b-col>
        </b-row>
        <div class="cardDiv">
            ...
            <div>
                <div class="app-container">
                    <div class="row justify-content-between">
                        <el-link style="margin-left: 20px; color: #409EFF;font-size: 18px;" href="/api/userManager/sampleUserExcel" :underline="false" download>导入模板下载</el-link>
                        <el-button style="margin: 10px;font-size: 18px;" class="col-2" type="primary" @click="excelSave" round>完&emsp;成</el-button>
                    </div>
                ...
                <b-form @submit="newUserSave" class="container pt-3">
                    ...
                    <b-form-group class="my-0 w-100">
                        <b-form-select v-model="newItem.position" class="mb-3">
                            <optgroup v-for="part in allData.positions" :key="part.label" :label="part.label">
                                <option v-for="pos in part.options" :key="pos.value" :value="pos.value">{{pos.text}}</option>
                            </optgroup>
                        </b-form-select>
                    </b-form-group>
                    ...
                    <b-button class="mt-3 my-4 col-5 float-right" block variant="primary"
                              @click="this.$refs['newUser'].hide()">取 消
                    </b-button>
                </b-form>
            </div>
        </b-modal>
    ...
    export default {
        ...
        methods: {
            queryCompanyUsers() {
                this.run();
                UserManageService
                    .queryCompanyUsers({...this.queryParam, ...this.queryDebounceParam})
                    .then(data => {
            ...
            handleSuccess({ results, header }) {
                if (!header.equals(["姓名", "用户名", "性别", "手机号", "邮箱", "部门", "职务"])) {
            ...
            excelSave() {
                ...
                UserManageService
                    .excelDataSave({'excelData': JSON.stringify(excelData)})
                    .then((res) => {
                        if (res.results === 'success')
                            UserManageService
                                .queryCompanyUsers({...this.queryParam, ...this.queryDebounceParam})
            ...
            handleDownload() {
                ...
                import('@/components/UploadExcel/Export2Excel').then(excel => {
                    const tHeader = ["姓名", "用户名", "性别", "手机号", "邮箱", "部门", "职务"];
                    const filterVal = ['name', 'username', 'gender', 'phone', 'email', 'part', 'position'];
            ...
            deleteUsers() {
                if (confirm("您确定要删除该用户吗？")){
                    this.run();
                    UserManageService
                        .deleteUsers({ids: JSON.stringify(this.selected)})
                        .then(res => {
                            if (res.results === 'success')
                                UserManageService
                                    .queryCompanyUsers({...this.queryParam, ...this.queryDebounceParam})
