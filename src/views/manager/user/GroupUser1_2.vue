            ...
            <b-col md="7" class="align-self-center">
                <b-button-group class="float-right" v-if="isActionAllowed('code_user_management', 'code_user_review_user')">
                    <b-button
                        ...
                        @click="setReview(1,null)"
                    >批量审核通过</b-button>
                    <b-button
                        class="styledBtn fontedBtn"
                        :size="template_size"
                        variant="outline-primary"
                        @click="setReview(2,null)"
                    >批量审核不通过</b-button>
                </b-button-group>
            </b-col>
        </b-row>
            ...
            >
                ...
                <template slot="action" slot-scope="row">
                    <b-button
                        ...
                        :size="template_size"
                        variant="outline-primary"
                        @click="setReview(1,row.item.id)"
                        v-if="isActionAllowed('code_user_management', 'code_user_review_user')"
                    >
                        <icon name="check"></icon>审核通过
                    </b-button>
                    <b-button
                        class="styledBtn"
                        :size="template_size"
                        variant="outline-primary"
                        @click="setReview(2,row.item.id)"
                        v-if="isActionAllowed('code_user_management', 'code_user_review_user')"
                    >
                        <icon name="times"></icon>审核不通过
                    </b-button>
                </template>
            </b-table>
        </div>
...
export default {
    ...
    methods: {
        queryDataList() {
            this.run();
            UserManageService.getGroupNonCompanyUsers({
                ...this.queryParam,
