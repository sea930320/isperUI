<template>
  <div class="function-action-wrapper">
    <div class="action-container clearfix">
      <a
        v-for="(action, index) in functionActions"
          ...
          <div class="detail max-hight-box">
            <table class="table table-gray table-striped table-hover table-border">
              ...
              <tbody>
                ...
                >
                  <td>
                    <a :href="file.url" class="btn-underline" target="_blank">{{file.filename}}</a>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  methods: {
    ...
    commitAction(action) {
      ...
      switch (action.cmd) {
        case actionCmd.ACTION_ROLE_SHOW:
          this.sendCMDMessage("入席", actionCmd.ACTION_ROLE_SHOW, null);
          break;
        case actionCmd.ACTION_ROLE_HIDE:
          this.sendCMDMessage("退席", actionCmd.ACTION_ROLE_HIDE, null);
          break;
        case actionCmd.ACTION_ROLE_STAND:
          this.sendCMDMessage("起立", actionCmd.ACTION_ROLE_STAND, null);
          break;
        case actionCmd.ACTION_ROLE_SITDOWN:
          this.sendCMDMessage("坐下", actionCmd.ACTION_ROLE_SITDOWN, null);
          break;
    ...
    roleLetoutOk() {
      ...
      this.sendCMDMessage(
        `送出 ${roleOutNames.join("、")}`,
        actionCmd.ACTION_ROLE_LETOUT,
        null,
        JSON.stringify(roleOutIds)
      );
      this.modalShow = 0;
    },
    // 申请约见确定
    ...
    submitDocOk(docs) {
      ...
      this.sendCMDMessage(
        "提交文件：" + docs.map(doc => doc.name).join("、"),
    ...
    bannedOk() {
      ...
      this.sendCMDMessage(
        ...
        null,
        JSON.stringify({ control_status: controlStatus })
      );
      this.modalShow = 0;
    },
    // 要求签字确定
    requestSignOk() {
      ...
      if (
        "docx".indexOf(
          selectDoc.filename
            .substring(selectDoc.filename.lastIndexOf(".") + 1)
            .toLowerCase()
        ) === -1
      ) {
    ...
    requestReportOk() {
      ...
      this.sendCMDMessage(
        ...
        null,
        JSON.stringify({
          role_alloc_id: this.reportRole.id,
          role_name: this.reportRole.name
        })
      );
      this.modalShow = 0;
    }
