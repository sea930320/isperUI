      ...
      <div v-show="!hasFlash" class="flash-tip-wrapper">
        <div class="flash-tip">
          ...
          <p>
            请点击
            <a
              href="//www.adobe.com/go/getflash?spm=a2h0j.8191423.movie_player.5~5~5~8~A"
              target="_blank"
              data-spm-anchor-id="a2h0j.8191423.movie_player.5~5~5~8~A"
            >这里</a>
          </p>
        </div>
      </div>
    ...
    actionHandler(action) {
      ...
      switch (action.ext.cmd) {
        ...
        case ACTION_ROLE_HIDE:
          this.flashObj.roleAction(
            `cmd=show&rolename=${action.ext.code_position}&operate=dvisible`
          );
          break;
        case ACTION_ROLE_STAND:
          this.flashObj.roleAction(
            `cmd=move&rolename=${action.ext.code_position}&direct=up`
          );
          break;
        case ACTION_ROLE_SITDOWN:
          this.flashObj.roleAction(
            `cmd=move&rolename=${action.ext.code_position}&direct=down`
          );
          break;
        case ACTION_ROLES_EXIT:
          action.ext.opt.data.forEach(role => {
            this.flashObj.roleAction(
              `cmd=show&rolename=${role.code_position}&operate=dvisible`
            );
          });
          break;
        case ACTION_TRANS:
          if (action.ext.code_position) {
            ...
            this.flashObj.roleAction(
              `cmd=speak&rolename=${action.ext.code_position}&says=${msgData}`
            );
            setTimeout(() => {
        ...
        case ACTION_ROLE_LETOUT:
          action.ext.opt.data.forEach(item => {
            this.flashObj.roleAction(
              `cmd=show&rolename=${item.code_position}&operate=dvisible`
            );
          });
          break;
        case ACTION_ROLE_TOWARD_REPORT:
          ...
          if (
            action.ext.opt.origin_position.sitting_status === 2 &&
            action.ext.opt.origin_position.is_self
          ) {
            this.flashObj.roleShow(
              action.ext.opt.origin_position.code_position,
              0
            );
          }
          roleCmd = this.flashObj.getRoleCmd(
            action.ext.opt.report_position.code_position,
            action.ext.opt.gender,
            action.ext.opt.actors,
            action.ext.opt.user.name
          );
        ...
        case ACTION_ROLE_END_REPORT:
          ...
          if (action.ext.opt.sitting_status === 2) {
            let roleCmd = this.flashObj.getRoleCmd(
              action.ext.opt.origin_position.code_position,
              action.ext.opt.gender,
              action.ext.opt.actors,
              action.ext.opt.user.name
