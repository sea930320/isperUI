    ...
    <div class="audio-player">
      <div class="audio-content clearfix">
        <div class="audio-play" @click="touchCover">
          <svg v-if="!state.playing" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="#1185fe" class="AudioPlayerSkin-controlButtonIcon-3HjX" data-name="play-new"><path fill-rule="evenodd" d="M17.6 11.43c.533.314.533.826 0 1.14L6.965 18.866c-.533.315-.965.06-.965-.57V5.704c0-.63.433-.885.965-.57L17.6 11.43z"></path></svg>
          <svg v-if="state.playing" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="#1185fe" class="AudioPlayerSkin-controlButtonIcon-3HjX" data-name="pause-new"><path fill-rule="evenodd" d="M15 6.5c0-.276.214-.5.505-.5h.99c.28 0 .505.23.505.5v11c0 .276-.214.5-.505.5h-.99a.506.506 0 0 1-.505-.5v-11zm-7 0c0-.276.214-.5.505-.5h.99c.28 0 .505.23.505.5v11c0 .276-.214.5-.505.5h-.99A.506.506 0 0 1 8 17.5v-11z"></path></svg>
        </div>
        <div class="audio-length fr">{{state.playing ? mu.state.currentTimeFormat : mu.state.durationTimerFormat}}</div>
      </div>
...
export default {
  ...
  computed: {
    progressWidth() {
      return this.mu.state.currentTime / this.mu.state.duration * 100 + '%'
    }
  },
  mounted() {
    this.init()
