	...
	function resizeToasts($toast, position) {
		...
		if (position.indexOf('toast-y-top') !== -1) {
			...
			$toasts.each(function(dom, index) {
				...
				$dom.css('top', top + toastHeight + gap + 'px')
			})
			...
			$toasts.each(function(dom, index) {
				...
				if (index === ($toasts.length - 1)) {
					$dom.css('margin-top', -(toastHeight / 2) + 'px')
				} else {
					var marginTop = parseFloat($dom.css('margin-top'))
					$dom.css('margin-top', marginTop + toastHeight + gap + 'px')
				}
			})
		} else {
			...
			$toasts.each(function(dom, index) {
				...
				$dom.css('bottom', bottom + toastHeight + gap + 'px')
			})
		}
	}
	...
	var Toast = {
		...
		show: function(options) {
			...
			if (icon) {
				...
				// <img class="tost-img" src="static/img/toast/' + options.type + '.svg" alt="">
				icon = '<div class="toast-icon ' + options.iconClass + '"></div>'
			} else {
				icon = ''
			}
