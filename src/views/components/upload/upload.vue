...
<div class="vue-upload-file" v-show="value">
	<div class="vuf-drop-area"
		...
		@drop="handleDrop">
		<i class="vuf-icon1 iconfont icon-upload2" v-show="loading != 1">
      ...
      for (let i = 0; i < fileNum; i++) {
        ...
        if (onlyImg && files[i].type.indexOf('image') === -1) {
          ...
          return false
				...
				// 格式限制
				if (that.ext.length !== 0 && that.ext.indexOf(files[i].name.substring(files[i].name.lastIndexOf('.') + 1).toLowerCase()) === -1) {
					that.hasError = true
...
<style lang='css' scoped>@import './upload.css';</style>
