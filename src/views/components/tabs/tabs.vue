...
export default {
    ...
    computed: {
        ...
        translateX() {
            return -(this.slideEls[0]
                ? this.clientWidth * (this.currentPage - 1)
                : 0);
        }
    },
    mounted() {
      ...
      if (childs) {
        ...
        this.$nextTick(function() {
            ...
            window.addEventListener(
                ...
                function() {
                    _this.clientWidth =
                        _this.$refs["tabswiper-wrap"].children[0].clientWidth;
                    var childs = _this.$refs["tabswiper-wrap"].querySelector(
                        ".tabswiper-wrap"
                    );
                    if (childs) {
                        let per = Math.round(
                            parseInt(
                                childs.style.transform.replace(/[^0-9]/gi, "")
                            ) / childs.children[0].clientWidth
                        );
                        childs.style.transform =
                            "translateX(" +
                            -(per * childs.children[0].clientWidth) +
                            "px)";
                    }
                },
                false
