  ...
  <div class="xml-modal-wrapper" v-show="visible">
    ...
    <div v-else class="overlay-box">
      <a
        ...
        @click="editMember"
      >编辑成员角色</a>
      <a href="javascript:;" class="quit-btn" @click="quit">{{!options.no_click?'退出业务':'关闭图表'}}</a>
    </div>
...
export default {
  ...
  methods: {
    createXml(xml) {
      ...
      if (
        self.options.parttern === 2 &&
        self.options.currentTask &&
        !self.options.no_click
      ) {
        // 双击选中元素
        ...
        eventBus.on("element.dblclick", function(e) {
          if (self.options.currentTask.length === 1) {
            ...
            if (!self.options.currentTask.includes(e.element.id)) {
              ...
              if (
                self.options.user_role_allocs[
                  self.options.currentTask.indexOf(e.element.id)
                ] === 0
              ) {
                self.$toasted.error("您不能参与该环节，请耐心等待！");
                return;
